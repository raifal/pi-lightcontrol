...
# copy to server
pscp -scp -l ${pi_zwave_user} -pw ${pi_zwave_ssh_passphrase} -i ..\secrets\raspberry-zwave.ppk scripts\restart.sh ${pi_zwave_ip}:~${pi_zwave_user}/
...
plink -l ${pi_zwave_user} -pw ${pi_zwave_ssh_passphrase} -i ..\secrets\raspberry-zwave.ppk ${pi_zwave_ip} chmod 700 ~${pi_zwave_user}/restart.sh
...
# start server
plink -l ${pi_zwave_user} -pw ${pi_zwave_ssh_passphrase} -i ..\secrets\raspberry-zwave.ppk ${pi_zwave_ip} ~${pi_zwave_user}/restart.sh
